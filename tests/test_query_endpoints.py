...
class TestQueryEndpoints(object):
    def test_queries(self, client, mock_db):
        query_result = {
            ...
            array([1, 2]),
            "query_text":
            array(["SELECT * FROM foo;", "SELECT * FROM bar;"]),
            "query_label":
            masked_array(data=["null label", "non null label"],
                         mask=[True, False]),
            "root_execution_id":
            array([1, 40])
        }
    ...
    def test_single_query(self, client, mock_db):
        ...
        mock_db.execute_query.assert_called_with(
            "SELECT * FROM query WHERE query_id=%(qid)s", {'qid': '1'})
        assert result_doc == doc
        assert response.status == falcon.HTTP_OK
    ...
    def test_single_query_nonexistent(self, client, mock_db):
        ...
        mock_db.execute_query.assert_called_with(
            "SELECT * FROM query WHERE query_id=%(qid)s", {'qid': '8'})
    ...
    def test_set_label(self, client, mock_db):
        ...
        mock_db.execute_query.assert_called_with(
            "UPDATE query SET query_label=%(label)s WHERE query_id=%(qid)s",
            dict([("label", "foo"), ("qid", "1")]))
        assert response.status == falcon.HTTP_OK
    ...
    def test_get_executions(self, client, mock_db):
        ...
        calls = [
            ...
            call(
                """SELECT root_execution_id FROM query WHERE query_id=%(qid)s""",
                {"qid": "1"})
        ]
